...
function M.open(opts)
    ...
    local lines = {
        ...
        "      Enter            선택 결과 열기",
        "      H                숨김(dotfile) 검색 포함/제외 토글",
        "      I                Git ignore 파일 검색 포함/제외 토글",
        ...
        "    Telescope H/I      dotfile / Git ignore 포함 여부 토글 후 검색 갱신",
        "    필터 공유          파일 트리 H/I와 같은 값을 사용하고 양쪽에 즉시 반영",
        "    필터 저장          stdpath(state)/file-tree-settings.json",
        "",
        "  Git",
        "",
        "    Space gg           LazyGit 열기",
        ...
        "    Space wv / ws      플로팅 슬롯을 좌우 / 상하 pane으로 분할",
        ...
        "    Ctrl-h/j/k/l       복수 pane: 그룹 내부 pane 사이 포커스 이동",
        "    Ctrl-Arrow         복수 pane: 방향키로 그룹 내부 포커스 이동",
        "    터미널 Ctrl 이동   normal mode 전환 후 플로팅 그룹 안에서 이동",
        "    Alt-h/l            단일 pane: 플로팅 창 너비 1칸 줄이기/늘리기",
        "    Alt-k/j            단일 pane: 플로팅 창 높이 1칸 늘리기/줄이기",
        "    Alt-h/j/k/l        복수 pane: 인접 pane 분할 비율 1칸 조절",
        "    Alt-Arrow          단일 크기 / 복수 pane 분할 비율 1칸 조절",
        "    Space Esc          기본 위치·크기와 모든 분할 비율 50:50 복구",
        "    Esc                터미널 모드에서 노멀 모드로 전환",
        "    Space bb           터미널 버퍼 선택/이동/삭제",
        "    :TKill             현재 터미널 종료",
        "    플로팅 슬롯        단일 [F1], 분할 시 [F1][A/B/...] 형식",
        "    전환 규칙          플로팅 슬롯은 하나만 표시, 새 슬롯을 열면 기존 슬롯 숨김",
        "    그룹 토글          같은 F 슬롯의 모든 pane을 함께 숨기고 복구",
        "    pane 닫기          남은 pane을 A/B/C 순서로 재정렬하고 영역 확장",
        "    세션 저장          pane 버퍼·분할 구조·비율·전체 위치·크기 저장/복원",
