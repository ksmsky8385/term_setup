...
local function configured_langs(langs)
    local configured = {}
...
M.config = function(_, opts)
    ...
    vim.api.nvim_create_user_command("TSMyInstall", function(opts)
        vim.cmd("TSInstall " .. vim.fn.fnameescape(opts.args))
    end, {
        nargs = 1,
    })

    vim.api.nvim_create_user_command("TSMyUninstall", function(opts)
        vim.cmd("TSUninstall " .. vim.fn.fnameescape(opts.args))
    end, {
        nargs = 1,
    })
    ...
    vim.api.nvim_create_user_command("TSMyList", function()
        ...
        local configured = configured_langs(opts.langs)
        local max_len = 0
        local function back()
            vim.cmd("bd")
            vim.cmd("TSSettings")
