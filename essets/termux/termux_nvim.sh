...
LIBUV_LINUX_C="$SRC_DIR/.deps/build/src/libuv/src/unix/linux.c"
...
if [ -f "$LIBUV_LINUX_C" ]; then
  grep -q "#include <limits.h>" "$LIBUV_LINUX_C" || \
    sed -i '1i #include <limits.h>' "$LIBUV_LINUX_C"
else
  echo "[ERROR] libuv source not found: $LIBUV_LINUX_C"
  exit 1
fi
